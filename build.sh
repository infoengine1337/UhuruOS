...
make_packages_repo() {
    ...
    local ROOT_GNUPG_HOMEDIR="${airootfs_dir}/root/.gnupg"
    if [ -d "${ROOT_GNUPG_HOMEDIR}" ]; then
      rm -rf "${ROOT_GNUPG_HOMEDIR}"
      install -d -m 600 "${ROOT_GNUPG_HOMEDIR}"
    else
      install -d -m 600 "${ROOT_GNUPG_HOMEDIR}"
    fi
...
chroot "${airootfs_dir}" /bin/bash << "EOC"
  ...
  #gpg --homedir=/root/.gnupg 2> /dev/null || false
