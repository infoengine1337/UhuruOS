...
#
# A simple script to run an uhurulive image using qemu. The image can be booted
# using BIOS or UEFI.
#
# Requirements:
# - qemu
...
Example:
    ...
    $ run_uhurulive -u -i uhurulive-2021.05.23-x86_64.iso
EOF
    printf '%s' "${usagetext}"
}
...
run_image() {
    ...
    qemu-system-x86_64 \
        ...
        -k en \
        -name uhurulive,process=uhurulive_0 \
...
secure_boot='off'
qemu_options=()
working_dir="$(mktemp -dt run_uhurulive.XXXXXXXXXX)"
    ...
    while getopts 'bdhi:su' flag; do
        case "$flag" in
            ...
            s)
                ...
                ;;
            *)
                printf '%s\n' "Error: Wrong option. Try 'run_uhurulive -h'."
                exit 1
                ;;
        esac
    done
